...
fn read(path: &str) -> Result<(Cache, Vec<u64>), Error> {
    ...
    Ok((Cache {addr_size, block_size, n_blocks, assoc, strat: None}, addrs))
} 

fn simulate(cache: &Cache, addrs: &Vec<u64>) -> Vec<Vec<CacheEntry>> {
    let mut result: Vec<Vec<CacheEntry>> = vec![vec![]; cache.n_blocks.try_into().expect("Block count too large for 32 bit machine.")];
    for i in 0..addrs.len() {
        let mut idx_mask: u64 = 0;
        for j in cache.block_size..cache.idx_bits() {
            idx_mask = idx_mask | 1 << j;
        }

        let mut tag_mask: u64 = 0;
        for j in (64 - cache.tag_bits())..64 {
            tag_mask = tag_mask | 1 << j;
        }
        ...
        for j in (set_idx * cache.block_size)..((set_idx + 1) * cache.block_size) {
            match result[j as usize].last() {
                Some(entry) => {
                    match lru {
                        Some(lru_idx) => {
                            if result[lru_idx as usize].last().unwrap().count_used < entry.count_used {
                                lru = Some(j);
                            }
                        }
                        None => lru = Some(j)
                    ...
                    match lfu {
                        Some(lfu_idx) => {
                            if result[lfu_idx as usize].last().unwrap().count_used < entry.count_used {
                                lfu = Some(j);
                            }
                        }
                        None => lfu = Some(j)
